...
<head>
  ...
  <body>
			  ...
			  <div class="col-sm-8 col-sm-offset-2">
				  <div class="panel panel-default">
					  ...
					  <form action="<c:url value='/TeacherServlet'/>" method="post">
						  <input type="hidden" name="method" value="addAbsent"/>
						  <input type="hidden" name="crID" value="${cr.courseID }"/>
						  <input type="hidden" name="crClass" value="${cr.courseClass}"/>
						  <input type="hidden" name="week" value="${cr.week}"/>
						  <input type="hidden" name="time" value="${cr.time}"/>
							  ...
							  <table class="table table-bordered table-striped table-hover" id="data_table">
								  ...
								  <c:forEach items="${requestScope.stList}" var="stList">
									  <tr>
										  ...
										  <td>${stList.stCourseAbsentTotal}</td>
										  <td>
											  <input type="checkbox" name="studentID" value="${stList.stID}"/>
										  </td>
									  </tr>
								  </c:forEach>
							  </table>
	  ...
	  <script type="text/javascript">
		  ...
		  var total_times = 0;
		  var total_stu = trs.length-1;
		  for(var i = 1 ; i < trs.length;i++)
		  {
			  var td = trs.eq(i).children().eq(4);
			  total_times+= parseInt(td.html());
		  }
		  var avg = total_times/total_stu;
		  for(var i = 1 ; i < trs.length;i++)
		  {
			  var td = trs.eq(i).children().eq(4);

			  if( parseFloat(td.html()) > parseFloat(avg)*3.0/2)
			  {
				  trs.eq(i).addClass("danger");
			  }
			  if(td.html()< avg*1.0/2)
			  {
				  trs.eq(i).addClass("success");
			  }
		  }
	  </script>
  </body>
