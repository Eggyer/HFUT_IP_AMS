      ...
      <div class="row">
          <div class="col-sm-12">
              <div class="panel panel-default">
                  <div class="panel-heading"><h4 class="centerh"><strong>已开课程</strong></h4></div>
                  <div class="table table-responsive">
                      <table class="table table-bordered table-striped table-hover">
                          ...
                          <c:forEach items="${requestScope.crList}" var="crList">
                              <tr>
                                  <td>${crList.courseID}</td>
                                  <td>${crList.crName}</td>
                                  <td>${crList.crAttribute}</td>
                                  <td>${crList.crCredit}</td>
                                  <td>${crList.courseClass}</td>
                                  <td>${crList.term}</td>
                                  <td>
                                      <a href="<c:url value='/TeacherServlet?method=preAddSt&crID=${crList.courseID}
          &crClass=${crList.courseClass}'/>">添加学生</a>
                                      <a href="<c:url value='/TeacherServlet?method=preChangeGrade&crID=${crList.courseID}
          &crClass=${crList.courseClass}'/>">录入成绩</a>
                                      <a href="<c:url value='/TeacherServlet?method=showTime&crID=${crList.courseID}
          &crClass=${crList.courseClass}'/>">添加缺勤次数</a>
                                      <a href="<c:url value='/TeacherServlet?method=delCtTime&crID=${crList.courseID}
          &crClass=${crList.courseClass}'/>">删除</a>
                                  </td>
                              </tr>
                          </c:forEach>
                      </table>
