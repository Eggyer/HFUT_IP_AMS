      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" align="center" style="border: 0px;">
            ...
            <h4 class="modal-title"  style=" color:#000;">编辑课程信息</h4>
          </div>
          <div class="modal-body">
				<form action="<c:url value='/AdminServlet'/>" method="post" style="text-align:center">
				<input type="hidden" name="method" value="editCourse"/>
						 	<div class="form-group">
                                 <label for="asIDid">课程编码</label>
								 <input type="hidden" name="courseID" value="${course.crID}"/>
								 <label>${course.crID}</label>
                            </div>
							<div class="form-group">
                                 <label for="asNameid">课程名称</label>
						         <input type="text" name="courseName" value="${course.crName}">
						         <label id="courseNameError" class="error"></label>
                            </div>	
                            <div class="form-group">
                                 <label for="bi">课程属性</label>
					             <input type="radio" name="courseAttribute" value="必修" id="bi" <c:if test="${course.crAttribute eq '必修' }">checked='checked'</c:if> />
					             <label for="bi">必修</label>
							     <input type="radio" name="courseAttribute" value="选修" id="bi" <c:if test="${course.crAttribute eq '选修' }">checked='checked'</c:if>/>
							     <label for="xuan">选修</label>
					             <label id="courseAttributeError" class="error"></label>
                            </div>
                            <div class="form-group">
                                 <label for="selector">课程学分</label>
          						 <input type="text" name="courseCredit" value="${course.crCredit}">
             					 <label id="courseCreditError" class="error"></label>
                            </div>
							<button type="button" class="btn btn-default btn-sm" onclick="add()">编辑</button>
