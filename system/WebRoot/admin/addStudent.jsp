...
window.onload = function() {
	var stid = document.getElementById("stIDid");
	var span = document.getElementById("stIDError");
		/*
		1. ajax
		*/
    stid.onblur = function()
    {
		ajax(
			{
				url:"<c:url value='/AdminServlet'/>?method=judgestID"+"&stID="+stid.value,
				type:"text",
	...
	function add() {
		$(".error").text("");
		var bool = true;
		if(!$(":text[name=stID]").val()) {
			$("#stIDError").text("学生学号不能为空");
			bool = false;
		}
		if(!$(":text[name=stName]").val()) {
			$("#stNameError").text("学生姓名不能为空");
			bool = false;
		}
		if($('#selector').val()==" ") {
			$("#stClassError").text("学生班级不能为空");
			bool = false;
		}
		if(!$(":text[name=stCollege]").val()) {
			$("#stCollegeError").text("学生学院不能为空");
			bool = false;
		}
		if(judge == 1)
		{
			$("#stIDError").text("学号已存在");
		    bool = false;	
		}
		if(bool) {
			$("form").submit();
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" align="center" style="border: 0px;">
            ...
            <h4 class="modal-title"  style=" color:#000;">添加学生</h4>
          </div>
          <div class="modal-body">
				...
				<input type="hidden" name="method" value="addStudent"/>
						 	<div class="form-group">
                                 <label for="stIDid">学生学号</label>
								 <input type="text" name="stID" id="stIDid"/>
								 <label id="stIDError" class="error"></label>
                            </div>
							<div class="form-group">
                                 <label for="stName">学生姓名</label>
          						 <input type="text" name="stName">
             					 <label id="stNameError" class="error">&nbsp;</label>
                            </div>	
                            <div class="form-group">
                                 <label for="selector">学生班级</label>
							     <select name="stClass" id="selector">
					               	<option value=" ">---请选择---</option>
					             	<c:forEach items="${requestScope.acLinkers}" var="acLinkers">
					               	<option value ="${acLinkers.stClass}">${acLinkers.stClass}</option>
					             	</c:forEach>
					             </select>
             					 <label id="stClassError" class="error"></label>
                            </div>
                           	<div class="form-group">
                                 <label for="stCollegeid">学生学院</label>
          						 <input type="text" name="stCollege" id="stCollegeid">
             					 <label id="stCollegeError" class="error"></label>
                            </div>		
