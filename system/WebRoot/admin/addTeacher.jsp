...
window.onload = function() {
	var tcid = document.getElementById("tcIDid");
	var span = document.getElementById("tcIDError");
		/*
		1. ajax
		*/
    tcid.onblur = function()
    {
		ajax(
			{
				url:"<c:url value='/AdminServlet'/>?method=judgetcID"+"&tcID="+tcid.value,
				type:"text",
	...
	function add() {
		$(".error").text("");
		var bool = true;
		if(!$(":text[name=tcID]").val()) {
			$("#tcIDError").text("教师工号不能为空");
			bool = false;
		}
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" align="center" style="border: 0px;">
            ...
            <h4 class="modal-title"  style=" color:#000;">添加教师</h4>
          </div>
          <div class="modal-body">
				<form action="<c:url value='/AdminServlet'/>" method="post" style="text-align:center">
						<input type="hidden" name="method" value="addTeacher"/>
						 	<div class="form-group">
                                 <label for="tcIDid">教师工号</label>
                                 <input type="text" name="tcID" id="tcIDid"/>
								 <label id="tcIDError" class="error"></label>
                            </div>
							<div class="form-group">
                                 <label for="tcNameid">教师姓名</label>
                                 <input type="text" name="tcName" id="tcNameid"  >
             					 <label id="tcNameError" class="error"></label>
                            </div>	
                            <div class="form-group">
                                 <label for="tcCollegeid">教师学院</label>
          						 <input type="text" name="tcCollege" id="tcCollegeid" >
