...
window.onload = function() {
	...
	var span = document.getElementById("stClassError");
		/*
		1. ajax
		*/
    stClass.onblur = function()
    {
		ajax(
			{
				url:"<c:url value='/AdminServlet'/>?method=judgeStClass"+"&stClass="+stClass.value,
				type:"text",
...
function add() {
	$(".error").text("");
	var bool = true;
	if(!$(":text[name=stClass]").val()) {
		$("#stClassError").text("班级不能为空");
		bool = false;
	}
	if(judge == 1)
	{
		$("#stClassError").text("班级已存在");
	    bool = false;
	}
	if(bool) {
		$("form").submit();
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" align="center" style="border: 0px;">
            ...
            <h4 class="modal-title"  style=" color:#000;">添加班级</h4>
          </div>
          <div class="modal-body">
				<form action="<c:url value='/AdminServlet'/>" method="post" style="text-align:center">
				<input type="hidden" name="method" value="addAcLinker"/>
						 	<div class="form-group">
                                 <label for="stClass">班级名字</label>
